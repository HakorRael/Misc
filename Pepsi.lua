				...
				if getsynasset then
					...
					elseif (#image > 14) and (string.sub(image, 1, 14) == "synasseturl://") then
						local x, e = pcall(function()
							...
							local fold = isfolder("./Cyclone.cc")
							if fold then
							else
								makefolder("./Cyclone.cc")
							end
							fold = isfolder("./Cyclone.cc/Themes")
							if fold then
							else
								makefolder("./Cyclone.cc/Themes")
							end
							fold = isfolder("./Cyclone.cc/Themes/SynapseAssetsCache")
							if fold then
							else
								makefolder("./Cyclone.cc/Themes/SynapseAssetsCache")
							end
							if not fold or not isfile("./Cyclone.cc/Themes/SynapseAssetsCache/" .. codename .. ".dat") then
								local res = game:HttpGet(string.sub(image, 15))
								if res ~= nil then
									writefile("./Cyclone.cc/Themes/SynapseAssetsCache/" .. codename .. ".dat", res)
								end
							end
							return getsynasset(readfile("./Cyclone.cc/Themes/SynapseAssetsCache/" .. codename .. ".dat"))
						end)
						if x and e ~= nil then
							return e
						end
...
function library:CreateWindow(options, ...)
	...
	if windowName and #windowName > 0 and library.WorkspaceName == "Cyclone.cc" then
		library.WorkspaceName = convertfilename(windowName, "Cyclone.cc")
	end
		...
		function tabFunctions:CreateSection(options, ...)
			...
			if isfolder and makefolder and listfiles and readfile and writefile then
				function sectionFunctions:AddPersistence(options, ...)
					...
					local realDropdownHolderList = Instance_new("UIListLayout")
					local dropdownEnabled = false
					if not isfolder("./Cyclone.cc") then
						makefolder("./Cyclone.cc")
					end
					local common_string = "./Cyclone.cc/" .. tostring(custom_workspace or library.WorkspaceName)
					local function resolvelist(nofold)
						if custom_workspace ~= options.Workspace then
							custom_workspace = options.Workspace
							common_string = "./Cyclone.cc/" .. tostring(custom_workspace or library.WorkspaceName)
						end
	...
	function windowFunctions:CreateDesigner(options, ...)
		...
		local daaata = {{"AddTextbox", "__Designer.Textbox.ImageAssetID", backgroundsection, {
			Name = "Image Asset ID",
			Placeholder = "rbxassetid://000000000",
